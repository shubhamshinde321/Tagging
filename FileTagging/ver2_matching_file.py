...
def create_table(path, table_name, log_file):
    ...
    try:
        ...
        df.insert(loc=4, column='existing_code', value=None)
        ...
        # cols = ['company_name', 'domain', 'Daily', 'existing_code', 'variant', 'industry']
        # df = df[cols]
        df.index += 1
        ...
        for data in result:
            ...
            for keyword in input_result:
                ...
                if key == company_name:
                    print(key)
                    print(code)
                    update_data_query = """Update transactions."{}" set existing_code = '{}' where company_name='{}' """.format(input_table, code, company_name)
...
def partial_tagging(result, input_table, keyword_table, file_name, log_file):
    ...
    try:
        select_new_input_query = """Select company_name from transactions."{}" where existing_code is null order by id""".format(input_table)
        ...
        for keyword in new_input_result:
            ...
            try:
                ...
                if match_found:
                    ...
                    cur.execute(insert_keyword_query, (raw_input_key, json_match_found),)
                    conn.commit()
...
if __name__ == '__main__':
    ...
    for data in result:
        ...
        if keyword_table_created:
            create_keyword_table(new_keyword_table)
